...
ORDER BY harga;`,
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'topic_20_2', title: 'Membaca Hasil EXPLAIN & Optimasi', content: `Memahami nilai kolom **type** adalah kunci membaca EXPLAIN:\n\n| type | Artinya |\n|------|--------|\n| ALL | Full table scan — terburuk |\n| index | Full index scan |\n| range | Index range scan — cukup baik |\n| ref | Index lookup by non-unique key |\n| eq_ref | Index lookup by unique key |\n| const | Satu baris (PRIMARY KEY) — terbaik |\n\n**Kolom Extra yang penting:**\n- \`Using filesort\` — butuh sorting tambahan, pertimbangkan index\n- \`Using temporary\` — butuh temp table, sering pada GROUP BY tanpa index\n- \`Using index\` — covering index, tidak baca data asli (bagus!)\n\n**EXPLAIN ANALYZE** (MySQL 8.0+) benar-benar menjalankan query dan menampilkan waktu aktual.`, codeExamples: [{ title: 'EXPLAIN ANALYZE & perbaikan query', code: `-- Sebelum optimasi: type=ALL karena fungsi pada kolom\nEXPLAIN SELECT * FROM artikel\nWHERE YEAR(diterbitkan) = 2024;\n-- type: ALL, rows: ~1000 (buruk!)\n\n-- Setelah optimasi: gunakan range pada kolom langsung\nEXPLAIN SELECT * FROM artikel\nWHERE diterbitkan BETWEEN '2024-01-01' AND '2024-12-31';\n-- type: range, rows: ~100 (jauh lebih baik)\n\n-- EXPLAIN ANALYZE menampilkan waktu aktual\nEXPLAIN ANALYZE\nSELECT a.judul, p.nama\nFROM artikel a\nJOIN penulis p ON a.penulis_id = p.id\nWHERE a.status = 'published'\nORDER BY a.diterbitkan DESC\nLIMIT 10;\n-- Output: -> Limit: 10 row(s) (actual time=0.5..0.5 rows=10 loops=1)\n--   -> Sort: a.diterbitkan DESC, limit input to 10 row(s) ...\n--     -> Nested loop inner join (actual time=0.3..0.4 rows=10 loops=1)` }] },
    ],
    summary: ['EXPLAIN menampilkan rencana eksekusi query tanpa menjalankannya', 'type=ALL berarti full table scan — perlu optimasi', 'Perhatikan kolom rows dan Extra untuk identifikasi bottleneck', 'EXPLAIN ANALYZE menampilkan waktu eksekusi aktual tiap operasi'],
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'topic_21_2', title: 'Mengelola VIEW & WITH CHECK OPTION', content: `**Mengelola VIEW:**\n- \`CREATE OR REPLACE VIEW\` — buat atau ganti view tanpa DROP terlebih dulu\n- \`ALTER VIEW\` — ubah definisi view\n- \`DROP VIEW\` — hapus view\n- \`SHOW CREATE VIEW\` — lihat definisi view\n\n**WITH CHECK OPTION** memastikan operasi INSERT/UPDATE melalui view hanya bisa memodifikasi baris yang memenuhi kondisi WHERE view tersebut. Tanpa ini, kamu bisa INSERT data yang tidak terlihat di view!\n\n**Updatable View:** View bisa di-INSERT/UPDATE jika tidak menggunakan GROUP BY, DISTINCT, aggregate function, subquery di SELECT, atau JOIN tertentu.`, codeExamples: [{ title: 'ALTER VIEW & WITH CHECK OPTION', code: `-- Ganti definisi view tanpa DROP\nCREATE OR REPLACE VIEW v_artikel_published AS\nSELECT a.id, a.judul, a.status, a.diterbitkan, p.nama AS penulis\nFROM artikel a\nJOIN penulis p ON a.penulis_id = p.id\nWHERE a.status = 'published';\n\n-- WITH CHECK OPTION: mencegah INSERT/UPDATE yang "keluar" dari view\nCREATE OR REPLACE VIEW v_artikel_draft AS\nSELECT id, judul, status, penulis_id\nFROM artikel\nWHERE status = 'draft'\nWITH CHECK OPTION;\n\n-- Ini akan ERROR karena status bukan 'draft'\n-- INSERT INTO v_artikel_draft (judul, status, penulis_id)\n-- VALUES ('Artikel Baru', 'published', 1);\n\n-- Lihat semua view\nSELECT TABLE_NAME, VIEW_DEFINITION\nFROM INFORMATION_SCHEMA.VIEWS\nWHERE TABLE_SCHEMA = DATABASE();` }] },
    ],
    summary: ['VIEW adalah query tersimpan yang bisa diperlakukan sebagai tabel', 'View tidak menyimpan data, query dasarnya dieksekusi tiap akses', 'Updatable view memungkinkan INSERT/UPDATE/DELETE ke view', 'WITH CHECK OPTION mencegah modifikasi data di luar kondisi WHERE view'],
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'topic_22_2', title: 'Control Flow & Cursor dalam Procedure', content: `**Control Flow dalam Procedure:**\n\n\`\`\`sql\nIF kondisi THEN ...\nELSEIF kondisi2 THEN ...\nELSE ...\nEND IF;\n\nCASE variabel\n  WHEN nilai1 THEN ...\n  ELSE ...\nEND CASE;\n\nWHILE kondisi DO ...\nEND WHILE;\n\nLOOP ... END LOOP; -- dengan LEAVE untuk break\n\`\`\`\n\n**CURSOR** memungkinkan iterasi baris per baris dari result set. Digunakan saat logika per-baris tidak bisa dilakukan dengan satu query set-based.\n\n**Urutan penggunaan CURSOR:**\n1. DECLARE cursor\n2. DECLARE CONTINUE HANDLER FOR NOT FOUND\n3. OPEN cursor\n4. FETCH dalam loop\n5. CLOSE cursor`, codeExamples: [{ title: 'CURSOR + LOOP dalam Procedure', code: `DELIMITER //\n\nCREATE PROCEDURE UpdateSkorPenulis()\nBEGIN\n  DECLARE v_done INT DEFAULT FALSE;\n  DECLARE v_penulis_id INT;\n  DECLARE v_total_artikel INT;\n  DECLARE v_total_views INT;\n\n  -- Deklarasi cursor\n  DECLARE cur_penulis CURSOR FOR\n    SELECT id FROM penulis;\n\n  -- Handler saat tidak ada data lagi\n  DECLARE CONTINUE HANDLER FOR NOT FOUND SET v_done = TRUE;\n\n  OPEN cur_penulis;\n\n  penulis_loop: LOOP\n    FETCH cur_penulis INTO v_penulis_id;\n    IF v_done THEN\n      LEAVE penulis_loop;\n    END IF;\n\n    -- Hitung statistik tiap penulis\n    SELECT COUNT(*), COALESCE(SUM(dilihat), 0)\n    INTO v_total_artikel, v_total_views\n    FROM artikel\n    WHERE penulis_id = v_penulis_id AND status = 'published';\n\n    -- Update skor\n    UPDATE penulis\n    SET artikel_count = v_total_artikel,\n        total_views = v_total_views\n    WHERE id = v_penulis_id;\n  END LOOP;\n\n  CLOSE cur_penulis;\nEND //\n\nDELIMITER ;\nCALL UpdateSkorPenulis();` }] },
    ],
    summary: ['Stored Procedure menyimpan logika SQL yang bisa dipanggil berulang', 'Parameter: IN (input), OUT (output), INOUT (keduanya)', 'DELIMITER harus diubah saat membuat prosedur dengan multiple statements', 'CURSOR memungkinkan iterasi baris per baris dalam stored procedure'],
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'topic_23_2', title: 'Function vs Procedure & Kapan Menggunakannya', content: `**Perbandingan Function vs Procedure:**\n\n| Aspek | Function | Procedure |\n|-------|----------|-----------|\n| Return | RETURNS satu nilai | OUT parameter / result set |\n| Dipanggil | Dalam SELECT/WHERE/SET | Dengan CALL |\n| Transaksi | Tidak bisa COMMIT/ROLLBACK | Bisa |\n| Efek samping | Tidak disarankan modifikasi data | Bisa DML |\n| Error handling | Terbatas | HANDLER lengkap |\n\n**Kapan pakai Function:**\n- Kalkulasi yang hasilnya dipakai dalam SELECT\n- Transformasi nilai (format tanggal, konversi satuan)\n- Business logic yang mengembalikan satu nilai\n\n**Kapan pakai Procedure:**\n- Operasi DML (INSERT/UPDATE/DELETE) kompleks\n- Proses batch\n- Logika yang butuh multiple result set`, codeExamples: [{ title: 'Function vs Procedure — perbandingan nyata', code: `-- FUNCTION: cocok untuk kalkulasi dalam SELECT\nDELIMITER //\nCREATE FUNCTION FormatRupiah(nominal DECIMAL(15,2))\nRETURNS VARCHAR(30)\nDETERMINISTIC\nNO SQL\nBEGIN\n  RETURN CONCAT('Rp ', FORMAT(nominal, 0, 'id_ID'));\nEND //\nDELIMITER ;\n\n-- Digunakan langsung dalam SELECT\nSELECT judul,\n       FormatRupiah(harga) AS harga_display\nFROM produk\nWHERE stok > 0;\n\n-- PROCEDURE: cocok untuk operasi DML kompleks\nDELIMITER //\nCREATE PROCEDURE ProsesPembayaran(\n  IN p_order_id INT,\n  IN p_jumlah DECIMAL(15,2),\n  OUT p_status VARCHAR(20)\n)\nBEGIN\n  DECLARE v_total DECIMAL(15,2);\n  SELECT total INTO v_total FROM orders WHERE id = p_order_id;\n  IF p_jumlah >= v_total THEN\n    UPDATE orders SET status = 'paid' WHERE id = p_order_id;\n    SET p_status = 'success';\n  ELSE\n    SET p_status = 'insufficient';\n  END IF;\nEND //\nDELIMITER ;\n\nCALL ProsesPembayaran(1, 150000, @status);\nSELECT @status;` }] },
    ],
    summary: ['Function mengembalikan nilai dan bisa digunakan dalam SELECT', 'DETERMINISTIC berarti output selalu sama untuk input yang sama', 'Gunakan READS SQL DATA atau MODIFIES SQL DATA sesuai operasi', 'Function cocok untuk kalkulasi; Procedure cocok untuk operasi DML kompleks'],
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'topic_24_2', title: 'BEFORE Trigger & Validasi Data', content: `**BEFORE trigger** sangat berguna untuk:\n1. **Validasi data** — batalkan operasi jika data tidak valid\n2. **Normalisasi data** — ubah nilai sebelum disimpan (misal: uppercase, trim)\n3. **Mengisi nilai otomatis** — set nilai kolom berdasarkan kolom lain\n\n**SIGNAL SQLSTATE** adalah cara melempar error kustom dari trigger (atau procedure/function). Operasi yang memicu trigger akan dibatalkan.\n\nFormat: \`SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'pesan error';\`\n\nKode \`'45000'\` adalah "generic user-defined error" — gunakan ini untuk error kustom.`, codeExamples: [{ title: 'BEFORE Trigger dengan validasi & normalisasi', code: `DELIMITER //\n\n-- Validasi: tolak artikel dengan judul terlalu pendek\nCREATE TRIGGER before_artikel_insert\nBEFORE INSERT ON artikel\nFOR EACH ROW\nBEGIN\n  -- Normalisasi: trim whitespace dari judul\n  SET NEW.judul = TRIM(NEW.judul);\n\n  -- Validasi panjang judul\n  IF LENGTH(NEW.judul) < 5 THEN\n    SIGNAL SQLSTATE '45000'\n    SET MESSAGE_TEXT = 'Judul artikel minimal 5 karakter';\n  END IF;\n\n  -- Validasi: status hanya boleh nilai tertentu\n  IF NEW.status NOT IN ('draft', 'published', 'archived') THEN\n    SIGNAL SQLSTATE '45000'\n    SET MESSAGE_TEXT = 'Status tidak valid. Gunakan: draft, published, atau archived';\n  END IF;\n\n  -- Set nilai default jika tidak diisi\n  IF NEW.dilihat IS NULL THEN\n    SET NEW.dilihat = 0;\n  END IF;\nEND //\n\nDELIMITER ;\n\n-- Ini akan ERROR karena judul terlalu pendek\n-- INSERT INTO artikel (judul, status, penulis_id)\n-- VALUES ('Hi', 'published', 1);` }] },
    ],
    summary: ['Trigger otomatis berjalan saat INSERT, UPDATE, atau DELETE', 'NEW merujuk baris baru, OLD merujuk baris lama', 'BEFORE trigger bisa mencegah operasi dengan SIGNAL SQLSTATE', 'SIGNAL SQLSTATE membatalkan operasi dan mengirim error kustom ke aplikasi'],
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'topic_25_2', title: 'Mengelola dan Memonitor Event', content: `**ALTER EVENT** memungkinkan kamu mengubah jadwal atau isi event tanpa DROP dan CREATE ulang.\n\n**Opsi ALTER EVENT:**\n- Ubah jadwal: \`ON SCHEDULE EVERY ...\`\n- Aktifkan/nonaktifkan: \`ENABLE\` / \`DISABLE\`\n- Ganti nama: \`RENAME TO nama_baru\`\n- Ubah isi: \`DO ...\`\n\n**ON COMPLETION:**\n- \`ON COMPLETION NOT PRESERVE\` (default) — event sekali jalan dihapus otomatis setelah selesai\n- \`ON COMPLETION PRESERVE\` — event tetap ada meski sudah selesai\n\nMonitor event melalui \`INFORMATION_SCHEMA.EVENTS\` untuk melihat status, jadwal berikutnya, dan riwayat eksekusi.`, codeExamples: [{ title: 'ALTER EVENT & monitoring', code: `-- Ubah interval event\nALTER EVENT backup_harian\nON SCHEDULE EVERY 12 HOUR\nSTARTS CURRENT_TIMESTAMP;\n\n-- Nonaktifkan event sementara\nALTER EVENT backup_harian DISABLE;\n\n-- Aktifkan kembali\nALTER EVENT backup_harian ENABLE;\n\n-- Ganti nama\nALTER EVENT backup_harian RENAME TO backup_semiharian;\n\n-- Pastikan event tidak dihapus setelah selesai\nALTER EVENT hapus_draft_lama\nON COMPLETION PRESERVE;\n\n-- Monitor semua event via INFORMATION_SCHEMA\nSELECT\n  EVENT_NAME,\n  STATUS,\n  EVENT_TYPE,\n  INTERVAL_VALUE,\n  INTERVAL_FIELD,\n  LAST_EXECUTED,\n  STARTS\nFROM INFORMATION_SCHEMA.EVENTS\nWHERE EVENT_SCHEMA = DATABASE();\n\n-- Hapus event\nDROP EVENT IF EXISTS backup_semiharian;` }] },
    ],
    summary: ['Event Scheduler menjalankan SQL secara terjadwal seperti cron', 'Bisa sekali jalan atau berulang dengan interval tertentu', 'Harus diaktifkan dulu dengan SET GLOBAL event_scheduler = ON', 'ALTER EVENT mengubah jadwal atau status event tanpa perlu drop dan recreate'],
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'topic_26_2', title: 'Isolation Level', content: `Isolation level menentukan seberapa terisolasi transaksi satu dari yang lain. Ada trade-off antara konsistensi dan performa.\n\n**4 Isolation Level (dari paling longgar ke paling ketat):**\n\n| Level | Dirty Read | Non-Repeatable Read | Phantom Read |\n|-------|-----------|--------------------|--------------|\n| READ UNCOMMITTED | ✓ (ada) | ✓ (ada) | ✓ (ada) |\n| READ COMMITTED | ✗ | ✓ (ada) | ✓ (ada) |\n| REPEATABLE READ | ✗ | ✗ | ✓ (ada) |\n| SERIALIZABLE | ✗ | ✗ | ✗ |\n\n**Default MySQL: REPEATABLE READ**\n\n- **Dirty Read:** membaca data yang belum di-commit transaksi lain\n- **Non-Repeatable Read:** baca dua kali dapat hasil berbeda (baris diupdate transaksi lain)\n- **Phantom Read:** baca dua kali dapat baris berbeda (baris baru ditambah transaksi lain)`, codeExamples: [{ title: 'Mengatur Isolation Level', code: `-- Lihat isolation level saat ini\nSELECT @@transaction_isolation;\n-- Output: REPEATABLE-READ (default MySQL)\n\n-- Set untuk session ini\nSET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED;\n\n-- Set untuk transaksi berikutnya saja\nSET TRANSACTION ISOLATION LEVEL SERIALIZABLE;\n\n-- Set global (semua koneksi baru)\nSET GLOBAL TRANSACTION ISOLATION LEVEL REPEATABLE READ;\n\n-- Contoh: READ COMMITTED untuk laporan (lebih fresh)\nSET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED;\nSTART TRANSACTION;\nSELECT SUM(total) FROM orders WHERE status = 'paid';\n-- Data ini mencerminkan semua commit terbaru\nCOMMIT;` }] },
      { id: 'topic_26_3', title: 'Locking & Deadlock', content: `**Row-level locking** terjadi saat transaksi memodifikasi baris — InnoDB secara otomatis mengunci baris tersebut.\n\n**Explicit locking:**\n- \`SELECT ... FOR UPDATE\` — exclusive lock, transaksi lain tidak bisa baca/update\n- \`SELECT ... FOR SHARE\` — shared lock, transaksi lain bisa baca tapi tidak update\n- \`NOWAIT\` — langsung error jika lock tidak bisa diperoleh\n- \`SKIP LOCKED\` — lewati baris yang terkunci (berguna untuk task queue)\n\n**Deadlock** terjadi saat dua transaksi saling menunggu lock dari satu sama lain. InnoDB mendeteksi deadlock secara otomatis dan melakukan rollback pada transaksi yang lebih "murah" dibatalkan.`, codeExamples: [{ title: 'FOR UPDATE, NOWAIT, SKIP LOCKED', code: `-- FOR UPDATE: kunci baris untuk diupdate\nSTART TRANSACTION;\nSELECT * FROM orders WHERE id = 1 FOR UPDATE;\n-- Transaksi lain tidak bisa UPDATE orders WHERE id = 1\nUPDATE orders SET status = 'processing' WHERE id = 1;\nCOMMIT;\n\n-- NOWAIT: jangan tunggu, langsung error jika terkunci\nSTART TRANSACTION;\nSELECT * FROM orders WHERE id = 1 FOR UPDATE NOWAIT;\n-- Error: 3572 - Statement aborted because lock(s) could not be acquired\n\n-- SKIP LOCKED: untuk task queue (ambil job yang tidak sedang diproses)\nSTART TRANSACTION;\nSELECT * FROM job_queue\nWHERE status = 'pending'\nORDER BY created_at\nLIMIT 1\nFOR UPDATE SKIP LOCKED;\n-- Setiap worker mengambil job yang berbeda tanpa konflik\n\n-- Cek deadlock di error log:\n-- SHOW ENGINE INNODB STATUS\\G\n-- Cari bagian "LATEST DETECTED DEADLOCK"` }] },
    ],
    summary: ['Transaksi menjamin operasi berjalan atomis — semua atau tidak sama sekali', 'COMMIT menyimpan perubahan, ROLLBACK membatalkan', 'SAVEPOINT memungkinkan rollback parsial dalam transaksi', 'REPEATABLE READ adalah isolation level default MySQL', 'SKIP LOCKED sangat berguna untuk implementasi task queue tanpa deadlock'],
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'topic_27_2', title: 'Operator Boolean & Tuning Relevansi', content: `**Operator Boolean Mode:**\n\n| Operator | Fungsi | Contoh |\n|----------|--------|--------|\n| + | Kata HARUS ada | \`+python\` |\n| - | Kata TIDAK BOLEH ada | \`-java\` |\n| * | Wildcard (akhir kata) | \`pyth*\` |\n| " " | Frase tepat | \`"machine learning"\` |\n| > | Tingkatkan relevansi | \`>python\` |\n| < | Turunkan relevansi | \`<tutorial\` |\n| () | Kelompok ekspresi | \`+(python mysql)\` |\n\n**WITH QUERY EXPANSION** (Natural Language Mode): MySQL melakukan dua pencarian — pertama dengan kata kunci asli, lalu memperluas dengan kata-kata dari hasil teratas. Berguna saat pencarian terlalu spesifik.\n\n**Minimum word length:** MySQL default tidak mengindex kata dengan panjang < 4 karakter (ft_min_word_len). Ubah di my.cnf jika perlu.`, codeExamples: [{ title: 'Boolean operators & Query Expansion', code: `-- Operator boolean yang dikombinasikan\nSELECT judul, MATCH(judul, konten) AGAINST(\n  '+mysql +(tutorial panduan) -iklan*'\n  IN BOOLEAN MODE\n) AS skor\nFROM artikel\nWHERE MATCH(judul, konten) AGAINST(\n  '+mysql +(tutorial panduan) -iklan*'\n  IN BOOLEAN MODE\n)\nORDER BY skor DESC;\n\n-- Pencarian frase tepat\nSELECT judul FROM artikel\nWHERE MATCH(judul, konten)\n  AGAINST('\"stored procedure\" +mysql' IN BOOLEAN MODE);\n\n-- WITH QUERY EXPANSION: perluas pencarian otomatis\nSELECT judul, MATCH(judul, konten)\n  AGAINST('database' WITH QUERY EXPANSION) AS skor\nFROM artikel\nWHERE MATCH(judul, konten)\n  AGAINST('database' WITH QUERY EXPANSION)\nORDER BY skor DESC\nLIMIT 10;\n-- Akan mengembalikan artikel tentang "database"\n-- DAN topik terkait seperti "SQL", "index", "transaksi"` }] },
    ],
    summary: ['FULLTEXT index jauh lebih cepat dari LIKE untuk pencarian teks', 'Natural language mode otomatis menghitung relevansi', 'Boolean mode mendukung operator khusus untuk pencarian presisi', 'WITH QUERY EXPANSION memperluas pencarian dengan topik terkait secara otomatis'],
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'topic_30_2', title: 'Fungsi JSON Lanjutan & JSON_TABLE', content: `**Fungsi JSON lanjutan yang penting:**\n\n- **JSON_ARRAYAGG()** — agregasi baris menjadi JSON array (seperti GROUP_CONCAT tapi menghasilkan JSON)\n- **JSON_OBJECTAGG()** — agregasi baris menjadi JSON object\n- **JSON_MERGE_PATCH()** — merge dua JSON, nilai dari JSON kedua override JSON pertama\n- **JSON_MERGE_PRESERVE()** — merge dua JSON, menggabungkan nilai yang konflik menjadi array\n- **JSON_TABLE()** — konversi JSON ke baris tabel relasional\n\n**JSON_TABLE** adalah game-changer: kamu bisa men-query nested JSON seolah-olah itu tabel relasional biasa, termasuk dengan JOIN.`, codeExamples: [{ title: 'JSON_ARRAYAGG, JSON_TABLE, JSON_MERGE_PATCH', code: `-- JSON_ARRAYAGG: kumpulkan data sebagai JSON array\nSELECT\n  p.nama AS penulis,\n  JSON_ARRAYAGG(\n    JSON_OBJECT('id', a.id, 'judul', a.judul, 'views', a.dilihat)\n  ) AS artikel_list\nFROM penulis p\nJOIN artikel a ON p.id = a.penulis_id\nWHERE a.status = 'published'\nGROUP BY p.id, p.nama;\n\n-- JSON_MERGE_PATCH: update partial JSON\nUPDATE produk_extended\nSET atribut = JSON_MERGE_PATCH(\n  atribut,\n  '{\"ram\": \"32GB\", \"gpu\": \"RTX 4060\"}'\n)\nWHERE id = 1;\n-- {\"ram\": \"32GB\", \"storage\": \"512GB SSD\", \"warna\": [...], \"gpu\": \"RTX 4060\"}\n\n-- JSON_TABLE: ubah nested JSON menjadi baris tabel\nSELECT pe.nama, jt.warna\nFROM produk_extended pe,\nJSON_TABLE(\n  pe.atribut,\n  '$.warna[*]' COLUMNS (\n    warna VARCHAR(50) PATH '$'\n  )\n) AS jt;\n-- Output:\n-- Laptop ASUS | hitam\n-- Laptop ASUS | silver` }] },
    ],
    summary: ['Tipe JSON MySQL memvalidasi dan menyimpan JSON secara efisien', 'Operator -> dan ->> untuk mengakses nilai JSON', 'JSON index virtual column memungkinkan indexing nilai JSON', 'JSON_TABLE mengkonversi nested JSON menjadi baris tabel relasional yang bisa di-JOIN'],
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'topic_31_2', title: 'Generated Column + JSON Index & Use Cases', content: `**Masalah:** Kamu tidak bisa langsung membuat index pada path JSON (misal \`atribut->>'$.ram'\`).\n\n**Solusinya: Generated Column sebagai "jembatan" untuk index JSON.**\n\nBuat STORED generated column yang mengekstrak nilai dari JSON, lalu index kolom itu. Performa query akan sama cepatnya dengan mengindex kolom biasa.\n\n**Use cases populer:**\n1. Index nilai JSON yang sering di-filter\n2. Kolom komputasi yang digunakan dalam laporan\n3. Full-text search pada kolom JSON\n4. Normalisasi sebagian dari data semi-structured`, codeExamples: [{ title: 'Index JSON via Generated Column', code: `-- Tabel dengan kolom JSON\nCREATE TABLE produk_catalog (\n  id INT PRIMARY KEY AUTO_INCREMENT,\n  nama VARCHAR(200),\n  spesifikasi JSON,\n  -- Generated column mengekstrak nilai dari JSON\n  ram_gb SMALLINT AS (CAST(spesifikasi->>'$.ram_gb' AS UNSIGNED)) STORED,\n  harga DECIMAL(12,2) AS (CAST(spesifikasi->>'$.harga' AS DECIMAL(12,2))) STORED,\n  -- Index pada generated column\n  INDEX idx_ram (ram_gb),\n  INDEX idx_harga (harga)\n);\n\n-- Insert data\nINSERT INTO produk_catalog (nama, spesifikasi) VALUES\n('Laptop A', '{\"ram_gb\": 16, \"harga\": 15000000, \"merk\": \"ASUS\"}'),\n('Laptop B', '{\"ram_gb\": 32, \"harga\": 25000000, \"merk\": \"Dell\"}'),\n('Laptop C', '{\"ram_gb\": 8, \"harga\": 10000000, \"merk\": \"Acer\"}');\n\n-- Query ini menggunakan idx_ram (bukan full table scan!)\nEXPLAIN SELECT nama, ram_gb, harga\nFROM produk_catalog\nWHERE ram_gb >= 16\nORDER BY harga;\n-- type: range (bukan ALL) karena index pada generated column` }] },
    ],
    summary: ['Generated column otomatis dihitung dari ekspresi', 'VIRTUAL: dihitung saat baca, tidak ada storage overhead', 'STORED: disimpan di disk, bisa diindex untuk performa', 'STORED generated column dari JSON path bisa diindex — performa sama dengan kolom biasa'],
